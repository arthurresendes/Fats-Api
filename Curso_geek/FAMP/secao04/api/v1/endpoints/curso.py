...
async def post_curso(curso: CursoSchema, db: AsyncSession = Depends(get_session)):
    ...
    await db.refresh(novo_curso)  # Added to get the ID from database
    return novo_curso
...
async def get_cursos(db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        return cursos  # Fixed: added return statement
...
@router.delete("/{curso_id}", status_code=status.HTTP_204_NO_CONTENT)  # Fixed: removed response_model
async def delete_curso(curso_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if curso_del:
            await session.delete(curso_del)  # Fixed: changed curso_id to curso_del
            await session.commit()
            # No return statement for 204 status
        else:
            raise HTTPException(detail="Curso não encontrado", status_code=status.HTTP_404_NOT_FOUND)
